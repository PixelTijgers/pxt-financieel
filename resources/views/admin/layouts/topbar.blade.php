    ...
    <div class="navbar-content">
        ...
        <ul class="navbar-nav">
            ...
            <li class="nav-item dropdown">
                @php
                    $getFiscalYears = \App\Models\FiscalYear::where('year', session()->get('fiscal_year'))->first('name')
                @endphp

                <a class="nav-link dropdown-toggle" href="#" id="yearDropdown" role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    <span class="ms-1 me-1 d-none d-md-inline-block">{{ $getFiscalYears->name }}</span>
                </a>

                <div class="dropdown-menu" aria-labelledby="yearDropdown">

                    @foreach(\App\Models\FiscalYear::orderBy('year', 'DESC')->limit(5)->get() as $ficalYears => $ficalYear)

                        <a href="{{ route('change.year', $ficalYear->year) }}" class="dropdown-item py-2"><span class="ms-1">{{ $ficalYear->name }}</span></a>
